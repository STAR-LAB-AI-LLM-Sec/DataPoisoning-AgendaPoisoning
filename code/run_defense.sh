#!/usr/bin/env bash
# =============================================================================
# 防守方终端脚本 —— AgendaBot 基准（Baseline：参会人来源守卫
# AttendeeOriginGuard）
#
# 交互式配置：
#   1) 选择评测模式：
#        - 真实大模型 Agent（本题目为提示词工程类，按评测规范使用真实大模型实测）
...
#     bash run_defense.sh
#
# 输出：AttendeeOriginGuard 面对 AttendeeInject / NoOpAttack 的防御胜率、
#       8 条基线任务误伤率与资格线判定（results/metrics.json）
# =============================================================================
set -euo pipefail
cd "$(dirname "$0")"
...
echo ">>> [run_defense] 防守方 Baseline 评测（AttendeeOriginGuard）"
read -rp "使用真实大模型 Agent 评测？[y/N] " USE_API
if [[ "${USE_API:-}" =~ ^[Yy] ]]; then
